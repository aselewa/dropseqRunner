...
solo.out.sum <- read.delim(paste0('output/',params$sampleID,'_Solo.out/GeneFull/Summary.csv'),sep=',', header=F, stringsAsFactors = F)
UMIperCell <- read.delim(paste0('output/',params$sampleID,'_Solo.out/GeneFull/UMIperCellSorted.txt'), sep=',', header=F, stringsAsFactors = F)
...
UMIperCell$r <- rev(rank(UMIperCell$V1))
UMIperCell$type <- ifelse(UMIperCell$r >= n, 'black','grey')
